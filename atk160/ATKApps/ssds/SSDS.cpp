...
void QA::Check()
{
   ...
   do {
      Talk("Are sure you want "+value+" ?");
      Listen(chk->gname);
      if (curtag == "command") {
         if (curwords == "HELP") {
            Talk("I am trying to confirm your last input");
            Talk("Please say Yes or No.  If No, you can also give the correct value");
         }else if (curwords == "CANCEL"){
            ok = true; status = cancelled;
         }
      }
...
void BuildRecogniser()
{
   ...
   AObsData *od = acode->GetSpecimen();//Ã»¿´¶®
