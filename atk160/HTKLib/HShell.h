...
/* !HVER!HShell: 1.6.0 [SJY 01/06/07] */
...
#define WIN32               /* Choices are UNIX or WIN32 *///这里我改过
#endif
...
/* ---- Map printf for environments which have no console ----  */

#define printf ATKprintf
#define fflush ATKfflush

int ATKprintf(const char *, ...);
int ATKfflush(FILE * f);
Boolean HasConsole(void);
void SetNoConsoleMode(void);
...
/*
   As for HError, print error message on stderr but don't abort.
   In ATK error messages are buffered instead of printed.
*/

int HRErrorCount(void);
/*
   Return number of error messages buffered
*/

int    HRErrorGetCode(int n);
char * HRErrorGetMess(int n);
/*
   Return the n'th errcode and message (n runs 1,2,3,...)
*/

void HRErrorReset(void);
/*
   Clear all buffered error messages
*/

void DisableSCPHandling(void);
...
ReturnStatus InitShell(int argc, char *argv[], const char *ver);
/*
...
void Register(const char *ver);
/*
   Register module version info.
...
/*
   Write/print the current configuration and usage
*/
...
HTime GetTimeNow();
/*
   TimeNow - in HTK units (accuracy/granularity not guaranteed)
*/
...
FILE *FOpen(const char *fname, IOFilter filter, Boolean *isPipe);
/*
...
/*
   ...
   of any character.
   ... WithLen is same but with specified length of buffer
...
/*
   Read a raw string (i.e. word upto next white-space) from src and
   store it in s
*/
...
void SubstFName(const char *fname, char *s);
/*
